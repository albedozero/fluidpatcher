...
Name "FluidPatcher (win x86_64)"

; The file to write
OutFile "setup_fluidpatcher_win64.exe"
...
InstallDir $PROGRAMFILES64\FluidPatcher
...
Section -Prerequisites
  nsExec::ExecToStack "py -3.10-64 -VV"
  Pop $0 ; return code
  Pop $1 ; command output
  StrCmp $0 "error" instPython ; py command didn't work
  ${StrContains} $0 "Python 3.10" $1 ; correct python version?
  StrCmp $0 "" 0 donePython
  instPython:
	MessageBox MB_YESNO "Python 3.10 is required for FluidPatcher.$\nYour system does not appear to have 64-bit Python 3.10 installed.$\nWould you like to install it now?" /SD IDYES IDNO failedPython
	SetOutPath $TEMP
    File "bundled\python-3.10.10-amd64.exe"
	ExecWait "$TEMP\python-3.10.10-amd64.exe"
	IfErrors 0 donePython
  failedPython:
	MessageBox MB_OK "Required Python version not installed. Exiting."
	Quit
  donePython:
  nsExec::ExecToStack "py -3.10-64 -m pip -q show oyaml wxpython"
  Pop $0 ; return code
  Pop $1 ; command output
  ...
  StrCmp $0 "" donePackages
    ...
    ExecWait "py -3.10-64 -m pip install -U oyaml"
	ExecWait "py -3.10-64 -m pip install -U wxpython"
...
Section "FluidPatcher (required)"
  ...
  IfFileExists "$DOCUMENTS\FluidPatcher\fluidpatcherconf.yaml" +7
    ...
    FileClose $0
  SetOverwrite off ; don't overwrite existing banks, soundfonts, midis
...
Section "Start Menu Shortcuts"
  ...
  SetOutPath $INSTDIR
  CreateShortcut "$SMPROGRAMS\FluidPatcher\Uninstall.lnk" "uninstall.exe" "" "uninstall.exe" 0
  CreateShortcut "$SMPROGRAMS\FluidPatcher\FluidPatcher.lnk" "pyw" '-3.10-64 fluidpatcher.pyw "$DOCUMENTS\FluidPatcher\fluidpatcherconf.yaml"' "$INSTDIR\assets\gfl_logo.ico" 0
  CreateShortcut "$SMPROGRAMS\FluidPatcher\README.lnk" "README.md"
...
Section "Desktop Shortcut"
  ...
  SetOutPath $INSTDIR
  CreateShortcut "$DESKTOP\FluidPatcher.lnk" "pyw" '-3.10-64 fluidpatcher.pyw "$DOCUMENTS\FluidPatcher\fluidpatcherconf.yaml"' "$INSTDIR\assets\gfl_logo.ico" 0
